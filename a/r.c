...
ty(a){ H("ty ");I t;
 if(!QL(a)) {
   if(QA(a)){H("ty=> ");R 0;}
        ...
        else if(a==MN(8)){H("ty=> ");R 3;}
             else if(a==MN(9)){H("ty=> ");R 2;}
                  else{H("ty=> ");R 1;} }
 if(0>(a=U(a))){H("ty=> ");R 0;}
 t=Qs||*X?((A)(Qs?Qs:*X))->t-Xt:0;
 if(!a){H("ty=> ");R t;}
 else if(a==1&&t>1||a==2&&t==4){H("ty=> ");R 1;}
      else{H("ty=> ");R 0;} }
Z me(n,f,a,b,c){H("me ");
 H("me->");E e=mm(n+2);
 e->n=n,e->f=f,*e->a=a;
 if(n>1)e->a[1]=b;
 if(n>2)e->a[2]=c;
 H("me=> ");R QP(a)&&(f==MN(8)&&QA(b)||f==MN(9)&&a!=MP(74))?(H("me->"),a=ee(e),*r=a,ef(ME(e)),a):ME(e);}
...
Z rf(){H("rf ");H("rf->");I a=rt(1),f;
 H("rf->");I r1=mr(); H("rf->");I r2=ty(f= *t); for(;r1&&r2>1;y=1)
  if(++t,a=ty(f)==2?me(1,f,a):me(2,f,a,rt(1)),y>1)prr(1);
 if(*t=='{'&&(!QN(a)||MN(6)==a)){H("rf->");int r3=rl(a); H("rf=>\n");R r3;}
 else {H("rf=> ");R a;} }
Z vs(e)E e;{DO(e->n,if(!vl(e->a[i]))R 0)R 1;}
peak(f){E e=XE(f);R QE(f)&&e->f==MN(9)&&*e->a==MP(74);}

Z as(a){H("as "); I z;E e; int tmp;
 if(!(pvl(a)||QE(a)&&(e=XE(a),peak(e->f)||(e->f==MN(7)?vs(e):
 e->f!=MP(36)&&e->f!=MP(74)&&rvl(e->a[e->f==MX(0)?0:e->n-1]))))){H("asA->");prr(5);}
 ++t; H("as->");tmp=re(); H("as->");z=me(2,MN(0),a,tmp);
 if(QV(a))XV(a)->t=y;
 H("as=> ");R z;}

Z re(){H("re "); I f,a,w; int res;
 H("reFa->");a=rf();
 H("re->");if(!mr()){H("re=> ");R a;}
 if(*t==MN(0)){H("re->"); res=as(a); H("reZb=> "); R res;}
 if(y>1){H("re1->   ");prr(1);}
 H("f:%ld y:%ld ",f,y);
 if(f=!y){H("reFb->"); f=rf(); if(y!=1){H("re2->");prr(2,a);}}
 H("reE->");w=re();
 if(y){H("re3->");prr(3);}
 if(f){H("re->"); res=me(2,f,a,w); H("reZc=> "); R res;}
 else {H("re->"); res=me(1,a,w);   H("reZd=> "); R res;} }
...
gt(v)V v;{H("gt ");
 if(Df&&!v->z&&v->e){
  A i=v->i&&!v->a?(A)nl:v->i;
  I z;
  H("gt->");E e=(E)ma(3);
  if(v->z=2,e->f=v->e,e->n=!!i)if(qz(*e->a=(I)i))i=0;
  if(Df==2){H("gt->");if(pa(MV(v)),NL,i)H("gt->"),pa(i),NL;}
  if(z=ez(ME(e))){
   aset(v,ic(z),i,0);
   if(Sf&&v->rff)dc(af4(v->rff,v->rfc,z,i,0,v));
   dc(z);}
  val(v),mv(e);}
 for(;~v->a;){H("gt->");err(4,MV(v));}
 H("gt=> "); R v->a;}
...
rd(b)I *b;{H("rd "); //H("rd b:%p *b:%ld ",b,*b);
 I i; A z; V v; Qs=0;
 for(u=t=b;*t&&*t!=':';++t);
 //H("t:%p *t:%ld ",t,*t);
 if(!*t){H("rd->");int res=rz(b);H("rd=> ");R res;} H("rdZ\n");
 for(r=t;*++r;);{A f;I p[999],n=1,r=b[1]=='[',*j,*k,*x=X,y=1,d=r||b[1]==':';
 if(d){if(r){if(b[3]!=']'||t!=b+4){H("rd9a->");prr(9);}b[3]=b[2],b[2]=*b,b+=2;}goto L;}
 if(t[-1]=='}'){if(t==b+3)t[-1]=*b,b=t-1;else{for(j=k=t;j>b;)*--k=*(j-=2);b=k;}goto L;}
 if(t-b<4){if(t-b==3)r=*b,*b=b[1],b[1]=r;goto L;}
 if(y=*b!='('){if(b[1]!='('){H("rd9b->");prr(9);}b[1]=b[3];b[3]=b[4];t[-2]=*b;}
 else{r=b[1];b[1]=b[2];b[2]=r;}y=t-b++-y;y=y==5?2:y==6?3:0;if(!y){H("rd9c->");prr(9);}
L:r=t-b;if(!r){H("rd9d->");prr(9);}
 if(QV(*b))v=XV(*b),Cx=v->cx,*b=MS(v->s);else if(QS(*b))v=vi(XS(*b),Cx);
 DO(r,if(!QS(b[i])&&b[i]!=')'){H("rd9e->");prr(9);})Qs=*b;
 if(y==3&&*XS(b[2])->n=='g')++y;u=b+1;if(r-1>MAXR){H("rdH->");prr(7);}
 for(*p=0,j=t;*++t;){if(*t==':'){H("rd9f->");prr(9);}if(*t==MN(0))n=str(t,j-b,b,n,p);}
